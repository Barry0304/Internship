...
class WordSegmentation(object):
    ...
    def segment(self, text, lower = True, use_stop_words = True, use_speech_tags_filter = False):
        ...
        # temp =[[],[]]
        # word_list = []
        # for w,p in zip(myws,mypos):
        #     if w not in util.sentence_delimiters:
        #         temp[0].append(w)
        #         temp[1].append(p)
        #     else:
        #         if ''.join(temp[0]) in text:
        #             marks=["COLONCATEGORY","COMMACATEGORY","DASHCATEGORY","DOTCATEGORY","ETCCATEGORY","EXCLAMATIONCATEGORY","PARENTHESISCATEGORY","PAUSECATEGORY","PERIODCATEGORY","QUESTIONCATEGORY","SEMICOLONCATEGORY","SPCHANGECATEGORY","WHITESPACE"] 
    ...
    def segment_sentences(self,myws,mypos, sentences,use_jeiba, lower=True, use_stop_words=True, use_speech_tags_filter=False):
        ...
        if use_jeiba :
            ...
            temp=[]
            for word,pos in zip(myws,mypos):
                if word not in util.sentence_delimiters :
                    if pos not in marks :
                        temp.append(word)
                else:
                    res.append(temp)
...
class Segmentation(object):
    ...
    def segment(self, text,myws,mypos,use_jeiba, lower = False):
        text = util.as_text(text)
        ...
        # print('res = ',len(words_all_filters))
        # for i,j in zip(sentences,words_all_filters):
        #     print(i)
        #     print("".join(j))
